...
function addCaptionToImage(image, caption) {
  ...
  let dirName = fileNameNoExt.replace(/[^0-9a-f]/gi, '')
  ...
  return gifFrames({
    ...
    .then((frameData) => {
      ...
      for (let i = 0; i < frameData.length; i++) {
        ...
        let frameImage = frame
          .getImage()
          .pipe(fs.createWriteStream(frameImageName))
    ...
    .then((images) => {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        let imageCaptionPromise = jimp
          .read(imageName)
          .then((tpl) =>
            tpl.clone().write(imageActiveName, (e) => (e ? console.log(e) : ''))
          )
          .then(() => jimp.read(imageActiveName))
          .then((tpl) => {
            return jimp
              .loadFont(jimp.FONT_SANS_64_WHITE)
              .then((font) => [tpl, font])
          })
          .then((data) => {
            tpl = data[0]
    ...
    .then((captions) => {
      ...
      const stream = pngFileStream(
        path.resolve(__dirname, 'tmp', dirName, '*-caption.png')
      )
        .pipe(
...
app.get('/blankimage/:query?/:reqId?', (req, res) => {
  ...
  axios
    .get(
      `http://api.giphy.com/v1/gifs/search?api_key=${APIKEYS.GIPHY}&q=${query}&limit=${limit}`
    )
    .then((resp) => {
...
app.get('/caption/:file/:caption', (req, res) => {
  ...
  addCaptionToImage(file, req.params.caption).then((file) => {
    ...
    res.sendFile(
      path.resolve(__dirname, 'tmp', req.params.file + '-caption.gif')
    )
  })
})
...
app.post('/image/:caption/:filename', (req, res) => {
  pool.query(
    `INSERT INTO images(caption, filename) VALUES ("${req.params.caption}", "${req.params.filename}-caption.gif");`,
    (err, results) => {
      if (err) console.log(err)
      res.send('').status(200)
    }
